...
int PreviewForm::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	Glyph* note = notepadForm->textEditingForm->note->Clone();
	if (notepadForm->textEditingForm->GetIsLockedHScroll() == TRUE) {
		CRect rect;
		notepadForm->textEditingForm->GetClientRect(rect);
		DummyManager dummyManager(note, notepadForm->textEditingForm->characterMetrics, rect.Width());
		Long i = 0;
...
void PreviewForm::OnCommandRange(UINT uID) {
	if (uID == IDT_BUTTON_PRINT) {
		NotepadForm* notepadForm = static_cast<NotepadForm*>(this->parent);
		notepadForm->textEditingForm->SendMessage(WM_COMMAND, MAKEWPARAM(IDM_FILE_PRINT, 0));
		this->OnClose();
	}
